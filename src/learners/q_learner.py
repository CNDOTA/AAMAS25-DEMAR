...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if self.mixer is not None:
            # chosen_action_qvals, vs = self.mixer(chosen_action_qvals, batch["state"][:, :-1])

            # FIXME 0826: record the gradients
            qtot_vals, vs = self.mixer(chosen_action_qvals, batch["state"][:, :-1])
            qtot_qi_gradients = th.autograd.grad(qtot_vals.mean(), chosen_action_qvals, retain_graph=True)[0]
            ...
            # if False:
                ...
                target_max_qvals, target_vs = self.target_mixer(target_max_qvals, batch["state"][:, 1:])
