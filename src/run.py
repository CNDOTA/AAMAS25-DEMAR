...
def run_sequential(args, logger):
    ...
    while runner.t_env <= args.t_max:
        ...
        if buffer.can_sample(args.batch_size):
            # if args.mixer == 'memix':
            #     if buffer.can_sample(args.batch_size * args.N):
            #         episode_sample = buffer.sample(args.batch_size * args.N)
            #         # Truncate batch to only filled timesteps
            #         max_ep_t = episode_sample.max_t_filled()
            #         episode_sample = episode_sample[:, :max_ep_t]
            #         for en_idx in range(args.N):
            #             this_episode_sample = episode_sample[args.batch_size * en_idx: args.batch_size * (en_idx + 1)]
            #             if this_episode_sample.device != args.device:
            #                 this_episode_sample.to(args.device)
            #             learner.train(this_episode_sample, runner.t_env, episode, en_idx)
            # else:
